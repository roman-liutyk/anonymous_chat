...
/// 
/// Calls the [AuthService.signIn] to authenticate the user.
/// 
/// If authentication succeeds, generates a JWT token and returns it with the user id in response.
/// 
/// Returns [Response] with code `200`, [token] and [user.id] to be sent back to the client.
/// 
/// Returns [Response] with code `401` when credentials for authentication were not provided.
/// 
...
Future<Response> _onPost(RequestContext context) async {
  try {
    ...
    if (email == null || password == null) {
      // Returns a response with failure if credentials for authentication were not provided.
